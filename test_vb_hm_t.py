...
model.load_state_dict(torch.load('./vb_hm_cause_kaggle/vb_hm_model.pt'))
# model.eval()
...
for idx, batch in tqdm(enumerate(dataloader)):
    ...
    print(f"Loss: {total_loss.item()}, KL Loss: {tot_kl/2}, Frobenius: {fn_loss}, mse: {mse_loss}")
    break  
...
for name, param in phi2.named_parameters():
    if name=='transformer.h.9.attn.c_proj.weight':
        print(param)
    
# phi1.load_state_dict(torch.load('./vb_hm_{}_kaggle/phi1.pt'.format(args.ablation.lower())))
# phi2.load_state_dict(torch.load('./vb_hm_{}_kaggle/phi2.pt'.format(args.ablation.lower())))
# projection_model.load_state_dict(torch.load('./vb_hm_{}_kaggle/projection_model.pt'.format(args.ablation.lower())))
...
x_cfs1_dataloader = DataLoader(x_cfs1, batch_size=32, shuffle=True, num_workers=10)


x_cfs0 = TensorDataset(x_cfs_0.cpu(),C0.cpu(),X0_ii.cpu(), X0_tti.cpu(), X0_am.cpu(), X0_ve.cpu(), X0_vtti.cpu(), X0_vam.cpu())
x_cfs0_dataloader = DataLoader(x_cfs0, batch_size=32, shuffle=True, num_workers=10)
...
autoencoder1.load_state_dict(torch.load('autoencoder1_vb_hm.pt'))
autoencoder0.load_state_dict(torch.load('autoencoder0_vb_hm.pt'))
...
for j in tqdm(range(len(L))):
    ...
    #     p_bertscore = []
    #     for p,q in zip(gen,[explanation[j]]*len(gen)):
    #         p_bertscore.append()
    ...
    #actual_label
    #print('predicted majority class ', maj)
    #for l, g in zip(outer_label, outer_gen):
...
if len(gen_vals)>0:
    torch.save(gen_vals, f'./{args.ablation}_hm_vb_gen_t.pt')
