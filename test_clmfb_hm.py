...
print('Loading models from Folder: ./clmfb_hm_{}'.format(args.ablation.lower()))

phi1.load_state_dict(torch.load('./clmfb_hm_{}/phi1.pt'.format(args.ablation.lower())))
phi2.load_state_dict(torch.load('./clmfb_hm_{}/phi2.pt'.format(args.ablation.lower())))
projection_model.load_state_dict(torch.load('./clmfb_hm_{}/projection_model.pt'.format(args.ablation.lower())))
...
def get_test_data(model):
    ...
    with torch.no_grad():
        for idx, i in tqdm(enumerate(te_loader)):
...
with torch.no_grad():
    reduced_vectors1 = autoencoder1.encoder(x_cfs_1.to(torch.float32).to('cuda')) + C1.squeeze().to('cuda')
...
if int(args.counterfactual_test)==0:
    torch.save(gen_vals, f'./generated_samples/{args.ablation}_hm_clmfb_gen.pt')
