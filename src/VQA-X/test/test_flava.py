...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return enc, gl, explanation, img_path, text, torch.squeeze(visual_embeds)
...
for epoch in range(epochs):  # Number of epochs
    ...
    for nos, batch in tqdm(enumerate(train_dataloader)):
        inputs, labels, a, b, c,vemb = batch
...
# torch.save(model.state_dict(), './flava_finetuned_vqax.pt')




# model.load_state_dict(torch.load('./flava_finetuned_vqax.pt'))
# model.eval()
...
print('Loading models from Folder: ./flava_vqax_{}'.format(args.ablation.lower()))


    
    
phi1.load_state_dict(torch.load('./flava_vqax_{}/phi2.pt'.format(args.ablation.lower())))
phi2.load_state_dict(torch.load('./flava_vqax_{}/phi2.pt'.format(args.ablation.lower())))
projection_model.load_state_dict(torch.load('./flava_vqax_{}/projection_model.pt'.format(args.ablation.lower())))
...
for i in tqdm(range(IPS1.shape[0])):
    ...
    IPS_ = torch.cat((IPS0,IPS2),dim=0)
    ips_ = ip0+ip2
    L_ = [0]*IPS0.shape[0] + [2]*IPS2.shape[0]
...
X1_pv = torch.stack([i['pixel_values'] for i in X1])
...
X2 = []
X_org2 = []
for i in tqdm(range(IPS2.shape[0])):

    #these are originally all 0 labels

    x = IPS2[i]
    with torch.no_grad():
        del ip2[i]['visual_embeds']
        c, fin = get_hidden(model, ip2[i])
        C2.append(c.detach())
    X_org2.append(x)
    #print(x.shape)
    ...
    x_cfs = []
    x_cf = []
    IPS_ = torch.cat((IPS0,IPS1),dim=0)
    L_ = [0]*IPS0.shape[0] + [1]*IPS1.shape[0]
    ips_ = ip0+ip1
    for j in range(IPS_.shape[0]):
        x_ = IPS_[j]
        x_cfs.append(x_)
...
X2_ii = torch.stack([i['input_ids'] for i in X2])
X2_tti = torch.stack([i['token_type_ids'] for i in X2])
X2_am = torch.stack([i['attention_mask'] for i in X2])
X2_pv = torch.stack([i['pixel_values'] for i in X2])
# X2_ve = torch.stack([i['visual_embeds'] for i in X2])
# X2_vtti = torch.stack([i['visual_token_type_ids'] for i in X2])
# X2_vam = torch.stack([i['visual_attention_mask'] for i in X2])
"""
...
x_cfs0 = TensorDataset(x_cfs_0.cpu(),C0.cpu(),X0_ii.cpu(), X0_tti.cpu(), X0_am.cpu(), X0_pv.cpu())
...
torch.save(autoencoder0.state_dict(), './flava_finetuned_vqax_autoencode.pt')
with torch.no_grad():
    #ae_n is for original label n
    # reduced_vectors2 = autoencoder2.encoder(x_cfs_2.to(torch.float32).to('cuda')) + C2.squeeze()
...
if int(args.counterfactual_test)==1:
    ...
    torch.save(gen_vals, f'./generated_samples/{args.ablation}_esnli_flava_gen.pt')
