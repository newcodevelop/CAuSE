...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        visual_embeds,_,_ = self.feat_vb[ip][0]
        ...
        return gl, explanation, img_path, text, torch.squeeze(visual_embeds)
...
for epoch in range(1):  # Number of epochs
    for idx, batch in enumerate(dataloader):
        ...
        break
# print('Saving models to Folder: ./qwenvl_hm_{}'.format(args.ablation.lower()))

# # torch.save(phi1.state_dict(), './flava_esnli_{}/phi1.pt'.format(args.ablation.lower()))
# torch.save(phi2.state_dict(), './qwenvl_hm_{}/phi2.pt'.format(args.ablation.lower()))
# torch.save(projection_model.state_dict(), './qwenvl_hm_{}/projection_model.pt'.format(args.ablation.lower()))
...
phi1.load_state_dict(torch.load('./qwenvl_hm_{}_F/phi2.pt'.format(args.ablation.lower())))
phi2.load_state_dict(torch.load('./qwenvl_hm_{}_F/phi2.pt'.format(args.ablation.lower())))
projection_model.load_state_dict(torch.load('./qwenvl_hm_{}_F/projection_model.pt'.format(args.ablation.lower())))
...
# print(dl_model)
torch.save(dl_model, "dl_model_hm_qwen.pt") 
...
"""
dl_model = torch.load("dl_model_hm_qwen.pt") 
...
if int(args.counterfactual_test)==1:
    ...
    torch.save(gen_vals, f'./generated_samples/{args.ablation}_hm_qwenvl_gen_F.pt')
