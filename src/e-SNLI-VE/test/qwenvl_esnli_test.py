...
from train_qwenvl_esnli import QwenVLWithClassifier  # import your wrapper class
import numpy as np
import random
...
# paths
output_dir = "./qwenvl_lora_classifier_esnli"
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return gl, self.explanation[idx], img_path, self.hypothesis[idx], torch.squeeze(visual_embeds)
...
dd_= {0: 'contradiction', 1: 'neutral', 2: 'entailment'}
def get_data():
    model.eval()
    explanation = []
    text = []
...
# print('Saving models to Folder: ./qwenvl_esnli_{}_F'.format(args.ablation.lower()))

# # torch.save(phi1.state_dict(), './flava_esnli_{}/phi1.pt'.format(args.ablation.lower()))
# torch.save(phi2.state_dict(), './qwenvl_esnli_{}_F/phi2.pt'.format(args.ablation.lower()))
# torch.save(projection_model.state_dict(), './qwenvl_esnli_{}_F/projection_model.pt'.format(args.ablation.lower()))
...
phi1.load_state_dict(torch.load('./qwenvl_esnli_{}_F/phi2.pt'.format(args.ablation.lower())))
phi2.load_state_dict(torch.load('./qwenvl_esnli_{}_F/phi2.pt'.format(args.ablation.lower())))
projection_model.load_state_dict(torch.load('./qwenvl_esnli_{}_F/projection_model.pt'.format(args.ablation.lower())))
    ...
    with torch.no_grad():
        for idx, i in enumerate(te_loader):
            ...
            if pred_lab==labels:
                ...
                L.append(pred_lab)
                explanation.append('The answer is {}'.format(dd_[pred_lab]))
...
# print(X0)
# print(0/0)
# print([i['input_ids'] for i in X0])
# print([i['attention_mask'] for i in X0])
# print(processor)
# print(0/0)
...
# print(dl_model)
torch.save(dl_model, "dl_model_esnli_qwen.pt") 
...
"""
dl_model = torch.load("dl_model_esnli_qwen.pt") 
...
for j in tqdm(range(len(L))):
    ...
    class_f = model.lin(torch.tensor(Cs_[j]).to(torch.bfloat16).to('cuda')).argmax().item()
    if class_f!=L[j]:
        continue
...
if int(args.counterfactual_test)==1:
    ...
    torch.save(gen_vals, f'./generated_samples/{args.ablation}_esnli_qwenvl_gen_F.pt')
